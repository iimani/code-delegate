...
def _parser() -> argparse.ArgumentParser:
    ...
    s = sub.add_parser("suppliers").add_subparsers(dest="action", required=True)
...
def main(argv: Optional[List[str]] = None, out: TextIO = sys.stdout, err: TextIO = sys.stderr) -> int:
    ...
    if args.command == "customers":
        ...
        if args.action == "add":
            supplier, e = app.supplier_service.create(args.name, args.email, args.country)
            if e:
                return fail(e.message)
            print(supplier.id, file=out)
        else:
            suppliers, _ = app.supplier_service.list()
            for s in suppliers:
                print(f"{s.id}\t{s.name}\t{s.email}\t{s.country}", file=out)
