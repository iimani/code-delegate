...
def register(router: Router, app) -> None:
    def create(req: Request) -> Response:
        try:
            product = app.product_service.create(req.body.get("sku", ""), req.body.get("name", ""),
    ...
    def get(req: Request) -> Response:
        try:
            product = app.product_service.get(req.params["id"])
        except ServiceError as exc:
            return error_response(exc)
    ...
    def list_all(req: Request) -> Response:
        ...
        return json_response([product_to_dict(p) for p in app.product_service.list(active_only=active_only)])

    def change_price(req: Request) -> Response:
        try:
            product = app.product_service.change_price(req.params["id"], str(req.body.get("price", "")))
        except ServiceError as exc:
            return error_response(exc)
        return json_response(product_to_dict(product))
    ...
    def deactivate(req: Request) -> Response:
        try:
            product = app.product_service.deactivate(req.params["id"])
        except ServiceError as exc:
            return error_response(exc)
        return json_response(product_to_dict(product))
    ...
    def stock(req: Request) -> Response:
        try:
            level = app.inventory_service.level(req.params["id"])
        except ServiceError as exc:
            return error_response(exc)
    ...
    def receive(req: Request) -> Response:
        ...
        try:
            level = app.inventory_service.receive(req.params["id"], quantity)
        except ServiceError as exc:
            return error_response(exc)
        return json_response(level_to_dict(level))
